...
### 10.0 可量化规则（来自 1:1 复刻工程与 Wiki 教程）

**[来源说明]** 本节数值来自一次系统检索的子代理整理。它的说明是：本机网络对 minecraft.wiki、
planetminecraft、reddit 与搜索引擎直连均返回 403 或 Cloudflare 挑战，因此它改走三条通路取证：
① Minecraft Wiki 教程的**中文镜像站**（后浪云 / 树叶云，含《屋顶的比例》《弧形屋顶》《墙壁类型》《创建调色板》正文）；
② **Wayback Machine**（取得 Planet Minecraft 尖拱教程、屋顶指南）；③ 可直接访问的建造者博客
（[Cathedral Talk 1:1 复刻](https://www.cathedraltalk.fm/minecraft/amienscathedral1-1)、[Craftdex](https://craftdex.net/articles/the-architecture-of-builds) 等）。

**我没有独立取回这些页面正文复核**，所以本节数值标注为 **[子代理·待复核]**。
其中"1 格 ≈ 1 米"的换算前提与最终比例建议，是本项目最该优先验证的一条。

| 规则 | 数值 | 用途 |
|---|---|---|
| 单位换算 | 1:1 复刻工程用 **1 格 ≈ 1 米** | 所有现实比例得以直接搬到方块 |
| 屋脊高 / 中殿净高 | **1.34 ~ 1.44** | 决定屋面要多陡 |
| 侧廊高 / 中殿净高 | **≈ 0.42** | 侧廊必须明显低于中殿 |
| 塔宽 / 立面宽 | **≈ 1/3** | 钟塔不要做窄条 |
| 塔高 / 塔宽 | **4.6 ~ 5.0** | 与本方案独立推出的 4.5~5.5 吻合 |
| 耳堂总宽 / 立面宽 | **1.7 ~ 2.1** | 十字形平面的横向尺度 |
| 尖拱圆径 | **= 2 ×（跨距含两端）+ 1**，且同一连续圆环必须同奇偶 | 跨 10 → 圆径 21；跨 22 → 圆径 45 |
| 45° 人字屋顶升幅 | **= 半宽**（20 格宽 → 升 10~11） | 屋面坡度的快速估算法 |
| 可用的坡度档位 | **63.4° / 45° / 26.5°** | 分别对应"极陡/标准/缓" |
| 多屋顶体系阈值 | 跨度 **> 15×15** 必须做多屋顶 | 单一大屋面会读成白板 |
| 墙面加厚的阈值 | 墙厚 **> 5 格**必须有深度层；**> 8 格**必须加柱 / 腰线 / 凹龛 | 与"进深面"规则配合 |
| 深度可叠加值 | 结构外凸 1 格 + 窗退 1 格 + 玻璃板再退 0.4375 格 ≈ **1.5 格阴影** | 阴影强度的估算 |
| 曲线最小可读半径 | **6 格** | 小于 6 格的弧线在远景读不出来 |
| 明暗分工 | 宏观渐变管远景，细节管近景 | 两层信息各管一个距离 |
| 纪念性建筑例外 | Wiki 明确"教堂这类纪念性建筑"不受住宅屋顶高度规则约束 | 允许做比民居更高的屋顶 |

**记录到的三处分歧**（不取一边）：
① 细节该多还是该少；② 教堂内部该亮还是该暗；③ 平面该用奇数宽度还是偶数宽度。

**[子代理·待复核]** 一条排除提醒：中文网络流传的"贴吧大神经验"类聚合文含明显伪造细节
（虚构的方块 ID 与无来源百分比）。本笔记此前引用的
[我的世界建筑技巧：贴吧大神经验](https://www.mczfw.com/blog/24963.html) 属于该类型，
其"进深面"论述方向性可用，但其中的具体数字不应采信。

**[子代理·对本项目最重要的一条·待复核]** 11 层楼板、每层 4 格，恰好等于一个真实中殿的净高。
因此建议：机器应移入**侧廊 / 廊台 / 塔楼 / 地下室**，让中殿恢复 **4~6 层通高**，
楼板线只落在**拱廊顶**与 **triforium 底**。这样机器层数与中殿的竖向分层就重合了，
不再需要"外面是教堂、里面是机柜"。

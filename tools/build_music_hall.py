...
from hall_material_names import NAMES

PROJECT = Path(__file__).resolve().parents[1]
OUTPUT = PROJECT / 'music_hall'
OFFSET = (29, 3, 40)
...
def load_modules():
    ...
    for number in range(1,12):
        ...
        root=read_nbt(PROJECT/'nbt_save'/filename)
...
def main():
    ...
    chunk_dir=OUTPUT/'structure_tiles_church'; chunk_dir.mkdir(exist_ok=True)
    manifest=[]
    ...
    for name,count in sorted(total.items(),key=lambda p:-p[1]):
        lines.append(f'| {NAMES.get(name,name)} `{name}` | {count} | {count//64}组+{count%64} | {groups["shell"][name]} | {groups["control"][name]} |')
    ...
    payload={'size':report['size'],'offset':OFFSET,'palette':palette,'blocks':compact,'report':report,'connections':connection_report}
    template=(PROJECT/'tools'/'music_hall_viewer.html').read_text(encoding='utf-8')
